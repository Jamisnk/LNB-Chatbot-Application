...
namespace Chatbot_Application
{
    public partial class CoverTime : Form
    {
        ...
        public CoverTime(string text)
        {
            ...
            cmd.Parameters.AddWithValue("@Worked", 0);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    string query = "SELECT Shift_Date from Shift WHERE Shift_Date = @ShiftDate AND EmployeeID != @EmpID and Worked = @Worked";
                    ...
                    if (readerReturnValue.HasRows == true)
                    {
                        ...
                        DialogResult result = MessageBox.Show("Would you like to cover the shift on " + textBox1.Text + " ?", "Confirmation", buttons);
                        ...
                        else
                        {
                            if (!string.IsNullOrEmpty(textBox2.Text))
                            {
                                ...
                                {
                                    conn.Open();
                                    string query2 = "SELECT ShiftID from Shift WHERE Shift_Date = @ShiftDate AND EmployeeID != @EmpID and Worked = @Worked";
                                    SqlCommand cmd2 = new SqlCommand(query2, conn);
                                    cmd2.Parameters.AddWithValue("@ShiftDate", textBox1.Text);
                                    cmd2.Parameters.AddWithValue("@EmpID", empID);
                                    cmd2.Parameters.AddWithValue("@Worked", 0);
                                    ...
                                    cmd4.Parameters.AddWithValue("@Worked", 1);
